...
class BasketState extends StateModule{
  ...
  async addToBasket(_id) {
    ...
    if (!exists) {
      ...
      let item = this.store.getState().catalog.items.find(item => item._id === _id);
      //Не уверен, что это стоило исправлять конечно, но на всякий случай сделал
      if (!item) { // фикс бага с Ф5, когда стартуем со страницы описания, и ни в каталоге, ни в корзине итемов еще нет
