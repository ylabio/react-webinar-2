...
import qs from "qs";
import changeCategories from "../../utils/change-categories";
...
class CatalogState extends StateModule {
  ...
  async setParams(params = {}, historyReplace = false) {
    ...
    const queryCategory = newParams.category
      ? `&search[category]=${newParams.category}`
      : "";
    const response = await fetch(
      `/api/v1/articles?limit=${newParams.limit}&skip=${skip}&fields=items(*),count&sort=${newParams.sort}&search[query]=${newParams.query}${queryCategory}`
    );
    ...
    this.setState({
      ...
      categories: changeCategories(jsonCategories.result.items),
      isLoading: false,
    });
