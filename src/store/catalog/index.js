...
class CatalogState extends StateModule{
  ...
  async setParams(params = {}, historyReplace = false){
    ...
    const categoryParams = newParams.category ? `&search[category]=${newParams.category}` : '';
    const skip = (newParams.page - 1) * newParams.limit;
    const response = await fetch(`/api/v1/articles?lang=ru&limit=${newParams.limit}&skip=${skip}&fields=items(*),count&sort=${newParams.sort}${categoryParams}&search[query]=${newParams.query}`);
