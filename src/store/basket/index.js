...
class BasketState extends StateModule{
   ...
   * @param itemObj Объект товара в случае, если каталог не загружен
   */
  addToBasket(_id, itemObj={}) {
    let sum = 0;
    ...
    if (!exists) {
      ...
      // Если передан объект, а в каталоге нет
      if(!item && JSON.stringify(itemObj) !== JSON.stringify({})) {
        items.push({...itemObj, amount: 1});
        sum += itemObj.price;
      } else {
