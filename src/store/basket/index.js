...
class BasketState extends StateModule {
  ...
  addToBasket(_id) {
    ...
    // (this.store.state.catalog.selectItem && this.getState().items.length === 0) 
    ...
    if (!exists && !this.store.state.catalog.selectItem) {
    ...
    else if (this.store.state.catalog.selectItem && !exists) {
      const item = this.store.state.catalog.selectItem;
      items.push({ ...item, amount: 1 });
