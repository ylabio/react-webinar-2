...
class CatalogState extends StateModule {
  ...
  async setParams(params = {}, historyReplace = false) {
    ...
    const response = await fetch(
      `/api/v1/articles?` +
        `limit=${newParams.limit}&skip=${skip}&fields=items(*),count` +
        `&sort=${newParams.sort}&search[query]=${newParams.query}${
          newParams.category && `&search[category]=${newParams.category}` // если категрии "Все", то не применять категорию при вызове
        }`
    );
