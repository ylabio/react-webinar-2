...
class CatalogState extends StateModule {
  ...
  async setParams(params = {}, historyReplace = false) {
    ...
    const response = await fetch(
      `/api/v1/articles?lang=ru&limit=${newParams.limit}&skip=${skip}&fields=items(*),count&sort=${newParams.sort}&search[query]=${newParams.query}${newParams.currentCategory === '' ? '' : `&search[category]=${newParams.currentCategory}`
      }`
    );
