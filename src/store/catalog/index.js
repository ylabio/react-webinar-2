...
class CatalogState extends StateModule {
  ...
  async setParams(params = {}, historyReplace = false) {
    ...
    // Переменная определяющая строку запроса по категории
    const categoryQuery = newParams.category !== '' ? `&search[category]=${newParams.category}` : '';
    ...
    const response = await fetch(`/api/v1/articles?limit=${newParams.limit}&skip=${skip}&fields=items(*),count&sort=${newParams.sort}&search[query]=${newParams.query}${categoryQuery}`);
