...
class BasketState extends StateModule{
  ...
  addToBasket(_id) {
    ...
    if (!exists) {
      ...
      const item = this.store.getState().catalog.items.length //если из списка товаров 
      ? this.store.getState().catalog.items.find(item => item._id === _id) //если из окна товара
      : this.store.getState().product.currenProduct
      console.log(item)
