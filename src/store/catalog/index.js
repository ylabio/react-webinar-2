...
class CatalogState extends StateModule {
  ...
  async load() {
    const response = await fetch(
      `/api/v1/articles?limit=${this.store.state.catalog.limit}&skip=0&fields=items(*),count`
    );
    const json = await response.json();
    const allPages =
      (json.result.count / 10) % 1 !== 0
        ? Math.trunc(json.result.count / 10) + 1
  ...
  async getPage(page) {
    const response = await fetch(
      `/api/v1/articles?limit=${this.store.state.catalog.limit}&skip=` +
        `${10 * (page - 1)}`
    );
    const json = await response.json();
    this.setState({
