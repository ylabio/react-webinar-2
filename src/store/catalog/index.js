...
class CatalogState extends StateModule{
  ...
  async setParams(params = {}, historyReplace = false){
    ...
    const cat = newParams.category ? `search%5Bcategory%5D=${newParams.category}` : '';
    const response = await fetch(`/api/v1/articles?${cat}&limit=${newParams.limit}&skip=${skip}&fields=items(*),count&sort=${newParams.sort}&search[query]=${newParams.query}`);
